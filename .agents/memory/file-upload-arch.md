...
description: How file uploads work in Jelite Tutor — API server multer proxy, URL construction, and the reusable ImageUpload component.
---
...
`artifacts/jelite-tutor/src/components/ui/ImageUpload.tsx` — reusable, supports:
...
## Wired up in
- `artifacts/jelite-tutor/src/routes/profile.tsx` — avatar_url (profile photo)
- `artifacts/jelite-tutor/src/components/admin/SiteSettingsManager.tsx` — logo_url (site logo)
- `artifacts/jelite-tutor/src/routes/admin.tsx` BlogManager — cover_url (blog post cover)
- `artifacts/jelite-tutor/src/routes/admin.tsx` SchoolsManager — logo_url (school logo)
