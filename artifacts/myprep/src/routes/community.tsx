...
import { MessageCircle, Plus, Loader2, Pin, Lock, Heart, ArrowUp } from "lucide-react";
...
const CATEGORIES = ["general", "JAMB", "WAEC", "NECO", "IELTS", "Schools", "Help"];
...
function CommunityPage() {
  ...
  const load = async () => {
    let q = supabase.from("forum_threads").select("*").order("pinned", { ascending: false }).order("created_at", { ascending: false }).limit(50);
  ...
  const create = async () => {
    if (!user) return toast.error("Log in to start a discussion");
    if (title.trim().length < 5 || body.trim().length < 10) return toast.error("Title 5+ chars, body 10+ chars");
    setCreating(true);
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2 bg-gradient-hero text-white"><Plus className="h-4 w-4" /> New thread</Button>
          </DialogTrigger>
          <DialogContent>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {threads.map((t) => (
            <div key={t.id} className="rounded-xl border border-border bg-card p-4 transition-colors hover:border-primary/40">
              <div className="flex items-start justify-between gap-3">
                <Link to="/community/$threadId" params={{ threadId: t.id }} className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
                    ...
                    <span>·</span>
                    <span className="flex items-center gap-1"><MessageCircle className="h-3 w-3" /> {t.reply_count}</span>
                  </div>
                </Link>
                <button
      ...
      <div className="mt-8 rounded-2xl border border-border bg-gradient-to-br from-primary/5 to-accent/5 p-5 text-center">
        <p className="text-sm font-medium">Want faster help?</p>
        <p className="mt-1 text-xs text-muted-foreground">Join our WhatsApp study groups</p>
        <a href="https://chat.whatsapp.com" target="_blank" rel="noopener noreferrer">
          <Button variant="outline" size="sm" className="mt-3 gap-2">
            <ArrowUp className="h-3 w-3 rotate-45" /> Join WhatsApp group
          </Button>
        </a>
      </div>
    </div>
