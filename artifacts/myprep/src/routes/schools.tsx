...
import { Search, MapPin, Loader2, GraduationCap } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-muted-foreground">Universities, polytechnics, colleges and more — with cutoff marks, fees and admission details.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map(s => (
            <div key={s.id} className="rounded-2xl border border-border bg-card p-5 hover:shadow-card-hover transition-all">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                    <MapPin className="h-3 w-3" /> {[s.location, s.state, s.country].filter(Boolean).join(", ")}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
                <div className="rounded-lg bg-secondary p-2"><p className="text-muted-foreground">Cutoff</p><p className="font-bold text-primary">{s.cutoff_score ?? "—"}</p></div>
                <div className="rounded-lg bg-secondary p-2"><p className="text-muted-foreground">Fees</p><p className="font-bold">₦{((s.fees_min ?? 0) / 1000).toFixed(0)}k+</p></div>
              </div>
